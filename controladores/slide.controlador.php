...
class ControladorSlide
{
	...
	static public function ctrActualizarSlide($datos)
	{
		...
		if ($datos["subirFondo"] != null) {
			...
			if ($datos["imgFondo"] != "vistas/img/plantilla/fondo.jpg") {
			...
			$directorio = "../vistas/img/sucursal/" . strtolower(str_replace('-', ' ', trim($_SESSION['nom_suc']))) . "/slide" . $datos["id"];
	...
	public function ctrEliminarSlide()
	{
		...
		if (isset($_GET["idSlide"])) {
			...
			if ($_GET["imgFondo"] != "vistas/img/plantilla/fondo.jpg") {
			...
			rmdir(strtolower(str_replace('-', ' ', trim($_SESSION['nom_suc']))) . "/slide" . $_GET["idSlide"]);
